...
class ModelWrapper:
    def __init__(self, model_name: str, device: torch.device, use_vllm: bool = False, args = None):
        ...
        if len(self.tokenizer) != self.model.get_input_embeddings().weight.shape[0]:
            ...
            if len(self.tokenizer) < self.model.get_input_embeddings().weight.shape[0]:
                pass  # model has more embeddings than tokenizer, safe to ignore
            else:
    ...
    def _build_latent_realign_matrix(self, model, device, args) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Move to target device after computation
        realign_matrix = realign_matrix.to(device=device)
        target_norm = target_norm.to(device=device)
    ...
    def _ensure_latent_realign_matrix(self, model, device, args) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if info is None:
            # If latent_space_realign is not enabled, skip expensive matrix computation
            # and just use identity + default norm
            if not self.latent_space_realign:
                d = model.config.hidden_size
                matrix = torch.eye(d, device=target_device, dtype=torch.float32)
                # Estimate target_norm from a small sample instead of full embedding
                input_embeds = model.get_input_embeddings()
                try:
                    w = input_embeds.weight.detach()[:100].to(device="cpu", dtype=torch.float32)
                    target_norm = w.norm(dim=1).mean().to(target_device)
                except (NotImplementedError, RuntimeError):
                    target_norm = torch.tensor(1.0, device=target_device)
            else:
